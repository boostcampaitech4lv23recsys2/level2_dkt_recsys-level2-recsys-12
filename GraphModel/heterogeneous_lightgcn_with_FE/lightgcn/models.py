...
def dropout_edge(user_problem_edge_index: Tensor,
    ...
    if not training or p == 0.0:
        ...
        return user_problem_edge_index, user_test_edge_index, user_tag_edge_index, user_feature_engineering, item_feature_engineering, edge_mask
    ...
    user_feature_engineering = user_feature_engineering.T[:, edge_mask].T
    item_feature_engineering = item_feature_engineering.T[:, edge_mask].T
    ...
    if force_undirected:
        ...
        user_feature_engineering = torch.cat([user_feature_engineering.T, user_feature_engineering.T.flip(0)], dim=1).T
        item_feature_engineering = torch.cat([item_feature_engineering.T, item_feature_engineering.T.flip(0)], dim=1).T
...
def build(
    ...
    if model_type == 0:
        ...
        pretrained_weight_path = f"/opt/ml/input/lightgcn_dev/lightgcn_modify_3/weight/pretrained_model.pt"
        ...
        inference_weight_path = f"./weight/n_epoch_{CFG.n_epoch}_feature_aggregation_method_{feature_aggregation_method_dict[CFG.feature_aggregation_method]}_feature_num_layers_{''.join(str(CFG.feature_num_layers))}_emb_dim_{CFG.embedding_dim}_model.pt"
...
def train(
    ...
    for e in range(1, n_epoch + 1):
        ...
        """
        if CFG.edge_dropout:
            user_problem_edge_index, user_test_edge_index,\
            user_tag_edge_index, out_src_feature_engineering,\
            out_dst_feature_engineering, edge_mask \
                = dropout_edge(
                    user_problem_train_data["edge"],
                    user_test_train_data["edge"],
                    user_tag_train_data["edge"],
                    train_user_feature_engineering,
                    train_item_feature_engineering,
                    p=CFG.edge_dropout_rate
                    )
            pred = model(user_problem_edge_index, user_test_edge_index, user_tag_edge_index, out_src_feature_engineering, out_dst_feature_engineering)
            loss = model.link_pred_loss(pred, torch.masked_select(user_problem_train_data["label"], edge_mask))
        else:
            pred = model(user_problem_train_data["edge"], user_test_train_data["edge"], user_tag_train_data["edge"], train_user_feature_engineering, train_item_feature_engineering)
            loss = model.link_pred_loss(pred, user_problem_train_data["label"])
        # backward
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        with torch.no_grad():
            prob = model.predict_link(user_problem_valid_data["edge"], user_test_valid_data["edge"], user_tag_valid_data["edge"], valid_user_feature_engineering, valid_item_feature_engineering, prob=True)
            ...
            if e == 1 or e % 100 == 0:
                ...
                pred_path = os.path.join(CFG.output_dir, f"tmp_pred_{CFG.n_epoch}_{''.join(str(CFG.feature_num_layers))}_{feature_aggregation_method_dict[CFG.feature_aggregation_method]}_emb_dim_{CFG.embedding_dim}.csv")
                ...
                logger.info(f"n_epoch: {CFG.n_epoch}, \t feature_num_layers: {CFG.feature_num_layers}, \t learning_rate: {CFG.learning_rate}, \t feature_aggregation_method: {feature_aggregation_method_dict[CFG.feature_aggregation_method]} \t emb_dim: {CFG.embedding_dim}")
                if CFG.edge_dropout:
            ...
            if model_type == 0 or model_type == 1:
                if auc > best_auc and acc > best_acc:
                    torch.save(
                        ...
                        f"./weight/n_epoch_{CFG.n_epoch}_feature_aggregation_method_{feature_aggregation_method_dict[CFG.feature_aggregation_method]}_feature_num_layers_{''.join(str(CFG.feature_num_layers))}_emb_dim_{CFG.embedding_dim}_model.pt"
                    )
def inference(
            model,
            user_problem_test_data,
            user_test_test_data,
            user_tag_test_data,
            out_src_feature_engineering,
            out_dst_feature_engineering,
            logger=None):
    model.eval()
    with torch.no_grad():
        pred = model.predict_link(
            user_problem_test_data["edge"],
            user_test_test_data["edge"],
            user_tag_test_data["edge"],
            out_src_feature_engineering,
            out_dst_feature_engineering,
            prob=True)
        return pred
