...
class LightGCN(torch.nn.Module):
    ...
    def get_embedding(self, edge_index: Adj) -> Tensor:
        ...
        breakpoint()
        """
        x.shape: torch.Size([16896, 64])
        out.shape: torch.Size([16896, 64])
        edge_index.shape: torch.Size([2, 1980184])
        """
        # layer들을 통과하는 부분입니다.
    ...
    def forward(self, edge_index: Adj,
        ...
        """
        # 우리 task는 edge_label_index is None임.
        # 따라서 그냥 edge_label_index = edge_index 이렇게 생각하면 됨
        if edge_label_index is None:
            if isinstance(edge_index, SparseTensor):
                edge_label_index = torch.stack(edge_index.coo()[:2], dim=0)
            else:
                edge_label_index = edge_index
        # torch.Size([userId 개수, embedding_dim])
        # k-hop layer 통과
        breakpoint()
        ...
        # breakpoint()
        # 차원이 같은 두 행렬을 가지고 새로운 정보를 얻으려면, nn.Linear로 키우고나서 nn.Conv / element-wise / sum
