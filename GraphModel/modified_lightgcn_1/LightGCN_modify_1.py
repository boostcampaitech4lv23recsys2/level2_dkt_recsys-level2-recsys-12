...
class LightGCN(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.feature_sorted_list = [torch.tensor(sorted_list, dtype=torch.int64, device= self.device)\
        ...
        if feature_weight is None:
            ...
            self.assessment_item_weight = 1. / (len(num_features_list) - 1)
            feature_weight = [1. / (len(num_features_list) - 1) for _ in range(len(self.num_features_list))]
        ...
        self.feature_embedding = ModuleList([Embedding(num_category, embedding_dim) for num_category in num_features_list[2:]])
    ...
    def get_embedding(self, edge_index: Adj) -> Tensor:
        ...
        self.final_feature_embedding = self.assessment_item_embedding.weight * self.assessment_item_weight
        
        for i in range(len(self.feature_weight)):
            self.final_feature_embedding += self.feature_embedding[i](self.feature_sorted_list[i]) * self.feature_weight[i]
